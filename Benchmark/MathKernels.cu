...
__global__ void CIEDE2000VariantDeltaTheta(float *colour1, float *colour2, double *result, int t_size)
{
    for (int i = 0; i < t_size; ++i)
    {
        constexpr double deg360InRad = degToRadKernel(360.0);
...
{
    CIEDE2000VariantDeltaTheta << <1, 1 >> > (colour1, colour2, result, t_size);
}

__global__ void CIEDE2000VariantS_L(float *colour1, float *colour2, double *result, int t_size)
{
    for (int i = 0; i < t_size; ++i)
    {
        constexpr double deg360InRad = degToRadKernel(360.0);
...
{
    CIEDE2000VariantS_L << <1, 1 >> > (colour1, colour2, result, t_size);
}