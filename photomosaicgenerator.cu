...
__global__
void euclideanDifferenceKernel(uchar *im_1, uchar *im_2, int size, int channels, double *variants)
{
...
__global__
void CIEDE2000DifferenceKernel(uchar *im_1, uchar *im_2, int size, int channels, double *variants)
{
    int index = (blockIdx.x * blockDim.x + threadIdx.x) * channels;
    int stride = blockDim.x * gridDim.x * channels;
    for (int i = index; i < size * channels; i += stride)
